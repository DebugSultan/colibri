...
class ConvertRoutingTest(unittest.TestCase):
    ...
    def test_an_output_directory_holding_a_checkpoint_is_refused(self):
        ...
        with tempfile.TemporaryDirectory() as out:
            ...
            # olmoe, not qwen4_exp: on this branch qwen38 declares an int4
            # container converter (convert_qwen38_int4.py), so its refusal
            # names the converter -- the glm5_next block below is that case.
            (Path(out) / "config.json").write_text(json.dumps({"model_type": "olmoe", "text_config": {}}))
