...
typedef enum {
    ...
    Q38_WEIGHT_BF16,
    Q38_WEIGHT_FP8,
    /* Nibble U8 (low nibble = even index), binary offset -8, per-GROUP
     * scales s[o*ng+g] with ng=(cols+gs-1)/gs — the layout of the int4 gs64
     * expert container written by convert_qwen38_int4.py and decoded by
     * matmul_i4_grouped().  weight->gs carries the group size. */
    Q38_WEIGHT_I4
} Q38WeightKind;

typedef struct {
    void *data;
    float *scales;                 /* block-FP8 and grouped-I4 */
    int rows, cols;
    int64_t elements, scale_count;
    Q38WeightKind kind;
    int gs;                        /* Q38_WEIGHT_I4 only */
    /* Device-side copy of this weight, owned by our VRAM tier and opaque here.
     * Only BF16 weights marked gpu_dense take it; everything else leaves it
     * NULL and the dispatcher below never looks at it. Named gpud to keep its
     * own: upstream's trunk path uses `gpu` for an int8 tier handle, and one
     * run drives exactly one of the two. */
    void *gpud;
...
typedef struct {
    Cfg c;
    shards S;
    shards S4;                         /* int4 expert container, Q38_INT4_SNAP */
    char prefix[32];
    ...
    int int4_active;                   /* S4 opened and serving experts */
...
static void q38_weight_free(Q38Weight *weight) {
    if(!weight)return;
    if(weight->gpud)q38t_dense_release(&weight->gpud);
...
static uint64_t q38_weight_bytes(const Q38Weight *weight) {
    ...
    if(weight->kind==Q38_WEIGHT_F32)
        data_bytes=(uint64_t)weight->elements*sizeof(float);
    else if(weight->kind==Q38_WEIGHT_BF16)
        data_bytes=(uint64_t)weight->elements*sizeof(uint16_t);
    else if(weight->kind==Q38_WEIGHT_FP8)
        data_bytes=(uint64_t)weight->elements;
    else if(weight->kind==Q38_WEIGHT_I4)
        data_bytes=(uint64_t)(weight->elements+1)/2;
    else data_bytes=0;
    return data_bytes+(uint64_t)weight->scale_count*sizeof(float);
}
...
static void q38_weight_matmul(float *y,const float *x,const Q38Weight *weight,
    ...
    if(weight->kind==Q38_WEIGHT_F32){
        ...
        if(weight->gpu_dense &&
           q38t_dense_matmul(&((Q38Weight*)weight)->gpud,y,x,
    ...
    }else if(weight->kind==Q38_WEIGHT_I4&&weight->scales&&weight->gs>0){
#ifdef __AVX2__
        if(q38_simd_matmul()){
            q38_matmul_i4_avx2(y,x,(const uint8_t*)weight->data,weight->scales,
                               weight->gs,S,I,O);
            return;
        }
#endif
        matmul_i4_grouped(y,x,(const uint8_t*)weight->data,weight->scales,S,I,O,
                          weight->gs);
    }else{
...
/* Two generations of qwen38 GPU tiers now live in this engine: upstream's
 * (qwen36_tier.c: fp8 experts streamed to VRAM as they get hot, int8 dense
 * trunk residents) and ours (qwen38_tier.c: int4 grouped experts in
 * per-device arenas with heat accounting, bf16 dense staged eager and
 * exact). They want the same VRAM and drive the same route masks, so
 * exactly one of them may run a token: Q38_TIER_TRUNK=1 selects upstream's,
...
static void model_init_range(Model *m,const char *snap,int cap,int bits,
    ...
    q38_load_cfg(&m->c,snap); q38_validate_cfg(&m->c); st_init(&m->S,snap);
    {
        ...
        if(i4snap&&*i4snap){
            st_init(&m->S4,i4snap);m->int4_active=1;
            fprintf(stderr,"[q38] int4 expert container: %s\n",i4snap);
        }
    }
...
static void q38_bind_borrowed_i4(Q38Weight *weight,void *data,float *scales,
                                 int rows,int cols,int gs) {
    q38_weight_free(weight);
    weight->data=data;weight->scales=scales;weight->rows=rows;weight->cols=cols;
    weight->elements=(int64_t)rows*cols;
    weight->scale_count=(int64_t)rows*((cols+gs-1)/gs);
    weight->gs=gs;weight->kind=Q38_WEIGHT_I4;
}
...
/* ---- int4 gs64 expert container (Q38_INT4_SNAP) ---------------------------
 *
 * convert_qwen38_int4.py writes, per expert, one merged_weight U8 tensor
 * (gate||up||down nibbles, row-major per matrix, binary offset -8) and one
 * qs F32 tensor (the concatenated per-GROUP scales s[o*ng+g]).  Both are
 * plain safetensors shards, so the container opens as a second store and the
 * loader geometry-checks each tensor before binding it into the slot.  The
 * group size is DERIVED from the qs byte count, never trusted from a meta
 * file: qb/4 floats total == 3*I*H/gs when gs divides both I and H, so a
 * container converted at a different gs binds at its true gs or not at all
 * (and the expert falls back to fp8). */

static int q38_int4_expert_tensors(Model *m,int layer,int expert,
                                   st_tensor *weight[2]) {
    if(!m->int4_active)return 0;
    char nm[320];
    snprintf(nm,sizeof nm,"%s.layers.%d.mlp.experts.%d.merged_weight",
             m->prefix,layer,expert);
    st_tensor *merged=st_find(&m->S4,nm);
    snprintf(nm,sizeof nm,"%s.layers.%d.mlp.experts.%d.qs",
             m->prefix,layer,expert);
    st_tensor *qs=st_find(&m->S4,nm);
    if(!merged||!qs)return 0;
    /* merged_weight U8 (dtype 3), qs F32 (dtype 2) — NOT 4/0: 4 is
     * F8_E4M3 and 0 is BF16, the first smoke ran on exactly this mistake
     * and silently fell back to serial fp8 loads. */
    if(merged->dtype!=3||qs->dtype!=2)return 0;
    weight[0]=merged;weight[1]=qs;
    return 1;
}

static int q38_int4_group_size(const Cfg *c,int64_t qs_bytes) {
    int64_t I=c->inter,H=c->hidden;
    int64_t lhs=(int64_t)12*I*H;          /* 3 matrices * I*H elements * 4 B */
    if(qs_bytes<=0||lhs%qs_bytes)return 0;
    int gs=(int)(lhs/qs_bytes);
    if(gs<8||gs>I||gs>H||H%gs||I%gs)return 0;
    if((int64_t)qs_bytes!=(2*(int64_t)I*(H/gs)+(int64_t)H*(I/gs))*4)return 0;
    return gs;
}
...
static void q38_bind_int4_slot(Slot *slot,void *merged,float *qs,
    ...
    q38_weight_free(&slot->down);
    if(!merged){
        int64_t slab_bytes=((int64_t)3*inter*hidden+1)/2+
                           (2*(int64_t)inter*(hidden/gs)+(int64_t)hidden*(inter/gs))*4;
        if(!slot->i4_slab||slot->i4_slab_bytes!=slab_bytes){
            void *replacement=realloc(slot->i4_slab,(size_t)slab_bytes);
            if(!replacement){fprintf(stderr,"OOM int4 expert slab\n");exit(1);}
    ...
    int64_t down_scales=(int64_t)hidden*(inter/gs);
    q38_bind_borrowed_i4(&slot->gate,merged,qs,inter,hidden,gs);
    q38_bind_borrowed_i4(&slot->up,(unsigned char*)merged+matrix_bytes,
                         qs+gate_scales,inter,hidden,gs);
    q38_bind_borrowed_i4(&slot->down,(unsigned char*)merged+2*matrix_bytes,
                         qs+gate_scales+down_scales,hidden,inter,gs);
}

static void q38_load_int4_ranges(Model *m,int layer,int expert,Slot *slot,
                                 st_tensor *weight[2]) {
    Cfg *c=&m->c;
    int gs=q38_int4_group_size(c,weight[1]->nbytes);
    if(!gs){fprintf(stderr,"int4 expert container geometry mismatch (layer %d)\n",layer);exit(1);}
    {
    ...
    q38_bind_int4_slot(slot,NULL,NULL,c->hidden,c->inter,gs);
    int64_t mb=weight[0]->nbytes;
    st_read_range_raw_cap(&m->S4,weight[0]->fd,weight[0]->off,mb,slot->i4_slab,
                          mb,1,"pread int4 merged_weight expert");
    st_read_range_raw_cap(&m->S4,weight[1]->fd,weight[1]->off,weight[1]->nbytes,
                          (unsigned char*)slot->i4_slab+mb,
                          slot->i4_slab_bytes-mb,1,"pread int4 qs expert");
}

static int q38_try_load_int4_expert(Model *m,int layer,int expert,Slot *slot) {
    st_tensor *weight[2];
    if(!q38_int4_expert_tensors(m,layer,expert,weight))return 0;
    double started=now_s();
    q38_load_int4_ranges(m,layer,expert,slot,weight);
    q38_tm_add(m,Q38_TM_EXPERT_READ,started);
    m->expert_weight_reads+=2;m->expert_pair_reads++;
    return 1;
}
 ...
 *
 * Direttiva del titolare (16/09): finché un esperto vive su SSD ogni misura
 * è invalida. Gli esperti fuori dal tier VRAM vengono letti da
 * q38_load_int4_ranges direttamente nelle mapping per-shard persistenti di
 * st_map_shard_range (COLI_MAP_EXPERTS=1): inchiodare quei range con mlock
 * rende la residenza deterministica — il kernel non li evicta, drop_caches
 * li rispetta, lo swap non li tocca — senza copie e senza toccare il
 * percorso caldo: gli stessi puntatori, solo pagine fissate.
 *
 * Il costo è il payload intero, 2,64 MiB/experto, e regge solo se i
 * residenti VRAM NON sono pinnati: l'universo intero (24.576 × 2,64 =
 * 64,8 GiB) non sta in RAM, mentre i ~13.477 non residenti del warmstart
 * (34,5 GiB) sì. Perciò il pass iniziale parte solo dopo un warmstart
 * reale (senza HEAT_FILE non c'è universo da coprire) e la coppia di
 * callback registrata sul tier (q38t_set_host_pin) mantiene da lì in poi
 * l'invariante locked <=> non-residente: unlock alla promozione (thread
 * uploader), relock al demote (vittima dello swap in q38t_offer).
 *
 * MLOCK=0 o Q38_HOST_PIN=0 disabilitano tutto; il default è ON perché è
 * la direttiva. Ogni mlock fallito è contato e non fatale: la parte non
 * pinnata resta su page cache, cioè lo status quo ante. */
...
static Model *g_pin_model;        /* single-model engine, come le globali del tier */
...
/* wire/unwire portabili: su POSIX è mlock/munload diretto (sys/mman.h arriva
 * da compat.h), su Windows la coppia compat_* (VirtualLock), altrove no-op. */
...
/* lock=1: inchioda merged-weight+scale dell'esperto; lock=0: sblocca. Gli
 * indirizzi sono deterministici (mapping per-fd a vita di processo), quindi
 * munlock ricade sugli stessi range di mlock. Il flag per-esperto evita il
 * munlock su pagine mai bloccate (che il kernel accetta in silenzio e che
...
static void q38_host_pin_expert(int layer,int expert,int lock){
    ...
    if(lock){
        ...
        for(int k=0;k<2;k++){
            ...
            /* Il cache morto va cacciato subito, non lasciato a kswapd: dopo
             * il warmstart ~29 GB di page cache appartengono ad esperti ora
             * residenti in VRAM, e la pressione della loro eviction lenta
             * manda in zram l'anonimo freddo (il banco ha visto "swappa
             * subito" durante il pass). DONTNEED su pagine pulite è
             * gratuito e non tocca il mapping: al prossimo accesso
             * refaultano dal disco, ma un esperto promosso non viene più
             * letto dall'host. */
#ifndef _WIN32
            if(t->fd>=0)
...
/* Upload eager del set denso al warmstart (dopo l'assestamento dell'arena):
 * ogni peso matmul-eligible col gate gpu_dense preso va in VRAM ORA, così il
 * primo forward non paga 725 upload HtoD sincroni dentro il prefill (il banco
 * r2048 lo ha mostrato: la via pigra funziona ma costa secondi nascosti nel
 * TTFT). embed non è nell'elenco — si legge a righe, non si moltiplica: il
 * fatto che la via pigra non lo chiami mai non autorizza a riempirci la VRAM.
 * Un rifiuto qui è lo stesso sticky-rifiuto della via pigra: il peso cade su
 * CPU come è sempre caduto. */
static int q38_dense_eager(Model *m){
    ...
    #define Q38EAGER(w) do{ Q38Weight *W=&(w); \
        if(W->kind==Q38_WEIGHT_BF16&&W->gpu_dense){ \
            if(q38t_dense_stage(&W->gpud,(const uint16_t*)W->data,W->cols,W->rows)) staged++; \
            else fell++; \
        } }while(0)
    Q38EAGER(m->lm_head);
...
static void q38_prefetch_native_fp8_experts(Model *m,int layer,
    ...
    if(m->int4_active){
        ...
        for(int index=0;index<count;index++){
            int expert=experts[index];st_tensor *weight[2];
            if(expert<0||expert>=m->c.experts||cache->by_expert[expert]>=0||
               !q38_int4_expert_tensors(m,layer,expert,weight))continue;
            posix_fadvise(weight[0]->fd,weight[0]->off,weight[0]->nbytes,
                          POSIX_FADV_WILLNEED);
            posix_fadvise(weight[1]->fd,weight[1]->off,weight[1]->nbytes,
                          POSIX_FADV_WILLNEED);
...
static int q38_expert_get_batch(Model *m,int layer,const int *experts,int count,
    ...
    for(int index=0;index<count;index++){
        ...
        if(slot_index>=0){
            ...
            if(m->int4_active){
                if(!q38_int4_expert_tensors(m,layer,expert,jobs[job_count].weight)||
                   !q38_int4_group_size(&m->c,jobs[job_count].weight[1]->nbytes)){
                    fprintf(stderr,"Qwen3.8 int4 expert layout changed during batch reservation\n");exit(1);
                }
            }else if(!q38_native_fp8_expert_tensors(m,layer,expert,jobs[job_count].weight)){
    ...
    if(job_count){
        ...
        for(int job=0;job<job_count;job++){
            if(m->int4_active)
                q38_load_int4_ranges(m,layer,jobs[job].expert,jobs[job].slot,
                                     jobs[job].weight);
            else
...
static void q38_attention(Model *m,Layer *l,int layer,const float *x,int S,int pos_base,float *out) {
    ...
    /* Prima la causa, poi il parallelismo: le scritture K/V/IK sono disgiunte
     * per posizione (ogni s scrive solo la propria riga) e devono essere
     * complete prima del ranking, che legge il prefisso IK[0..pos] intero. */
    #pragma omp parallel for schedule(static)
    for(int s=0;s<S;s++){
        int pos=pos_base+s;
        for(int h=0;h<KVH;h++){
    ...
    /* Ranking e attenzione sono indipendenti per posizione: nessuna scrittura
     * condivisa (heads e' a righe disgiunte, lo scratch e' per-filo) e nessun
     * ordine FP cambia dentro una posizione, quindi il risultato e' bit-identico
...
static void q38_tier_start(Model *m,int cap) {
    ...
    /* Ours owns the cards unless upstream's tier is explicitly chosen:
     * two tiers cannot budget the same VRAM (see q38_tier_choice). */
    if(!q38_tier_choice())return;
    Cfg *c=&m->c;
    if(!m->native_fp8){
...
static void q38_moe_decode(Model *m,Layer *l,int layer,const float *x,int S,float *out) {
    ...
    for(int s=0;s<S;s++){
         ...
         *
         * Both tiers that can own this loop speak the same mask language
         * (bit z set = route k z computed on the GPU), so one body drives
        ...
        for(int n=0;n<nn;n++){
            ...
            if(trunk){
                /* Upstream's accounting: the tier copies what it needs during
                 * the note, so the slot may be recycled right after. */
                q38_tier_note(layer,need[n],ex);
            }else{
         ...
         * the misses. */
        if(gmask){ if(trunk) qt_take(gmask,route_gates,K,ys); else q38t_take(gmask,route_gates,K,ys); }
...
static void q38_model_free(Model *m) {
        ...
        if(m->cache) {
            if(m->cache[i].slots) {
                for(int s=0;s<m->cache[i].n;s++) {
                    ...
                    free(m->cache[i].slots[s].i4_slab);
                }
            }
    ...
    free(m->ple_history); free(m->PLE_conv_state); free(m->c.is_attn);
    if(m->int4_active)st_destroy(&m->S4);
    st_destroy(&m->S);
    memset(m,0,sizeof(*m));
}
